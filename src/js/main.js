...
export default function () {
  ...
  function drawGdp(data, series, keys) {
    ...
    //Define right Y axis
    let yAxisR = d3.axisRight().scale(yScale).ticks(8).tickSizeOuter(0);

    //Define left Y axis
    let yAxisL = d3.axisLeft().scale(yScale).ticks(8).tickSizeOuter(0);
    ...
    const rects = bars
      ...
      .on("mousemove", function (d, i) {
        ...
        tooltipDiv
          .html(
            `${tooltipType}<br/>${formatTime(d.data.year)}: ${
              d.data[tooltipType]
            }%`
          )
    ...
    rects.on("click", function (d, i) {
      ...
      d3.csv("data/growth_data_" + thisType + ".csv", rowConverter).then(
        (thisGdpData) => {
          ...
          yScale = d3
            .scaleLinear()
            .domain([
              d3.min(transitionSeries, stackMin) - 0.5,
              d3.max(transitionSeries, stackMax) + 0.5,
            ])
            .range([h - margin.bottom, margin.top])
            .nice();
  ...
  function drawBackbutton(data, series, keys) {
    ...
    const backButton = svg
      ...
      .attr(
        "transform",
        `translate(${xScale.range()[0]},${yScale.range()[1]})`
      );
    ...
    backButton.append("text").attr("x", 22).attr("y", -10).html("&larr; Back");
    ...
    backButton.on("click", function () {
      ...
      svg.select(".axis.yl").transition().duration(barTransition).call(yAxisL);

      svg.select(".axis.yr").transition().duration(barTransition).call(yAxisR);
  ...
  function toggleBackButton() {
    ...
    if (viewState == 1) {
      ...
      const rectWidth = Math.round(
        backButton.select("text").node().getBBox().width + 16
      );
